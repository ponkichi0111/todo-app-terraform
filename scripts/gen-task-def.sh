...
DB_SECRET_ARN=$(terraform -chdir=infra/terraform/app output -json | jq -r '.rds_master_secret_arn.value')
...
IRA=$(terraform -chdir=infra/terraform/app output -json | jq -r '.ecs_app_task_role_arn.value')
IRT=$(terraform -chdir=infra/terraform/app output -json | jq -r '.ecs_task_execution_role_arn.value')
